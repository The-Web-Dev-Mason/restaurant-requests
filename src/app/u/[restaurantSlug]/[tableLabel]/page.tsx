...
export default function CustomerPage({ params }: PageProps) {
  ...
  useEffect(() => { fetchRestaurantAndTable() }, [])
  useEffect(() => { const interval = setInterval(() => updateCooldownTimers(), 1000); return () => clearInterval(interval) }, [cooldowns])
  ...
  const checkCooldowns = async (tableId: string) => {
    ...
    for (const option of requestOptions) {
      ...
      if (lastRequest && lastRequest.length > 0) {
        ...
        const now = new Date()
        newCooldowns[option.type] = now < cooldownUntil ? { until: cooldownUntil, timeLeft: getTimeUntil(cooldownUntil) } : { until: null, timeLeft: '' }
    ...
    setCooldowns(prev => {
      ...
      for (const type of Object.keys(updated) as RequestType[]) {
        if (updated[type].until) {
          ...
          if (timeLeft !== updated[type].timeLeft) { updated[type] = { ...updated[type], timeLeft }; hasChanges = true; if (timeLeft === '') updated[type] = { until: null, timeLeft: '' } }
        }
      }
  ...
  const submitRequest = async (requestType: RequestType) => {
    ...
    try {
      ...
      setCooldowns(prev => ({ ...prev, [requestType]: { until: cooldownUntil, timeLeft: getTimeUntil(cooldownUntil) } }))
